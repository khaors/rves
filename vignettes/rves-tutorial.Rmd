...
$$
where $T(\omega)$ is the Fourier transform of the resistivity transform, $G(\omega)=\mathcal{F}(\rho_{\text{app}}(x))$ is the Fourier transform of the apparent resistivity data, and $H(\omega)=\mathcal{F}(J_{1}(1/\exp{(x)}))$ is the Fourier transform of the Bessel function of order 1. In the Geophysics literature, this Fourier transform is called the Transform of the linear filter. This filter can be obtained from the ratio of the Fourier transform of $T(\lambda)$ and 
the Fourier transform of the $\rho_{\text{app}}(x)$
